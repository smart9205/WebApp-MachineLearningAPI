...
const TeamPlayerControl = ({ team }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', border: '1px solid #E8E8E8', borderRadius: '8px', width: '400px', height: '70vh', padding: '16px 12px' }}>
                {loading ? (
                    <div style={{ width: '100%', height: '80%', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <CircularProgress />
                    </div>
                ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'auto auto', gap: '8px', maxHeight: '55vh', overflowY: 'auto' }}>
                        {playerList.map((item, index) => (
                            ...
                            >
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '90px' }}>
                                    <img src={item.image ? item.image : PLAYER_ICON_DEFAULT} style={{ height: '60px' }} />
                                </div>
