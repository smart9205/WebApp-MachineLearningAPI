...
export const createCommand = (tagList, url, name) => {
  ...
  const command = `ffmpeg -y -i "${url}" -f lavfi -i color=color=#808080@0.7:size=540x60,format=rgba -f lavfi -i color=color=#FFA500@0.7:size=540x60,format=rgba -i "https://s3.eu-west-1.amazonaws.com/scouting4u.com/IMG/JustSmallLogo.png" -filter_complex "${tagList.map((tag, i) =>
